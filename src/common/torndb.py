...
def checkout(dbapi_con, con_record, con_proxy):
    """
    resolve mysql gone away issue
    http://docs.sqlalchemy.org/en/latest/core/events.html#sqlalchemy.events.PoolEvents
    :param dbapi_con:
    :param con_record:
    :param con_proxy:
    :return:
    ...
    try:
        ...
        if ex.args[0] in (2006, 2013, 2014, 2045, 2055):
            sys.stderr.write("connection lost with Ex: %s, refill with new connection\n" % ex)
            raise exc.DisconnectionError()
        else:
            raise


def get_pool(db_args, pool_size=10, max_overflow=25):
    poolkey = ''.join(['%s%s' % item for item in db_args.items()])
    if poolkey not in _POOLS:
        ...
        event.listen(_POOLS[poolkey], "checkout", checkout)
...
class Connection(object):
    ...
    def __init__(self, host, database, user=None, password=None,
                 max_idle_time=7 * 3600, connect_timeout=10, pool_size=10, max_overflow=25,
