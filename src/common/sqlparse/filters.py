...
import common.sqlparse
from common.sqlparse import sql, tokens as T
from common.sqlparse.compat import u, text_type
from common.sqlparse.engine import FilterStack
from common.sqlparse.pipeline import Pipeline
from common.sqlparse.tokens import (Comment, Comparison, Keyword, Name, Punctuation, String, Whitespace)
from common.sqlparse.utils import memoize_generator
from common.sqlparse.utils import split_unquoted_newlines
    ...
    def process(self, stack, stream):
        ...
        for token_type, value in stream:
            ...
            # We are processing an AS keyword
                ...
                # if (token_type == Name or token_type == Keyword or token_type == Name.Builtin) and not parenthesis:
                if token_type not in Whitespace and not parenthesis:
                    # We check also for Keywords because a bug in SQLParse
                    sql = formula
                    formula = ""
                    yield sql, value
                    mode = 1
                #if token_type != Name and not parenthesis:
                #    raise common.sqlparse.SQLParseException('keyword can not by column alias')
        if parenthesis:
            raise common.sqlparse.SQLParseException('parenthesis pair not match')
